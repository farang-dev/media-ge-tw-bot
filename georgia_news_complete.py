...
POSTED_ARTICLES_FILE = "posted_articles_complete.json"
...
def generate_short_summary(title, content=None, max_length=100):
    """Generate a very short summary for Twitter"""
    try:
        ...
        # Extract the first sentence or two from the content
        sentences = []
        remaining_length = max_length
        
        # Split content into sentences
        content_parts = content.replace('。', '。|').replace('.', '.|').replace('!', '!|').replace('?', '?|').split('|')
        ...
        for part in content_parts:
            ...
            if not (part.endswith('。') or part.endswith('.') or part.endswith('!') or part.endswith('?')):
                part = part + '。'
        ...
        if sentences:
            ...
            # If we couldn't extract sentences, just use the title
            if not title.endswith('。') and not title.endswith('.'):
                return f"{title}。"
            return title
            
    except Exception as e:
        debug_print(f"Error generating short summary: {str(e)}")
        if not title.endswith('。') and not title.endswith('.'):
            return f"{title}。"
        return title

def generate_ai_summary(title, content, max_length=150):
    """Generate a summary using OpenRouter API"""
    try:
        debug_print(f"Generating AI summary for: {title[:50]}...")

        # Check if OpenRouter API key is available
        if not os.getenv('OPENROUTER_API_KEY'):
            debug_print("No OpenRouter API key found, using local summarization")
            return generate_short_summary(title, content, max_length)
...
タイトル: {title}"""
        ...
        try:
            ...
            if 'choices' in data and data['choices']:
                ...
                debug_print(f"AI summary: {summary}")
                return summary
        except Exception as e:
...
def create_tweet(summary, url):
    """Create a tweet with the summary and URL, ensuring it fits within Twitter's character limit"""
    # Calculate maximum summary length to fit in a tweet
    max_summary_length = MAX_TWEET_LENGTH - len(url) - 5  # 5 for "\n\n" and buffer
    
    # If summary is already short enough, use it as is
    if len(summary) <= max_summary_length:
        return f"{summary}\n\n{url}"
    
    # If summary is too long, we need to truncate it
    ...
    if end_positions:
        ...
        truncated_summary = summary[:last_end + 1]
        return f"{truncated_summary}\n\n{url}"
    
    # If no sentence ending found, just truncate and add a period
    truncated_summary = truncated.strip() + '。'
    return f"{truncated_summary}\n\n{url}"

def main():
    debug_print("=" * 50)
    debug_print("Starting Georgia News Bot (COMPLETE VERSION)...")
    debug_print("=" * 50)
    ...
    # Generate AI summary
    summary = generate_ai_summary(selected_article['title'], content)
    debug_print(f"Generated summary: {summary}")
    
    # Create tweet
    tweet_text = create_tweet(summary, selected_article['url'])
    ...
    if success:
        ...
        if error_type == "duplicate":
            ...
            modified_tweet = create_tweet(new_summary, selected_article['url'])
